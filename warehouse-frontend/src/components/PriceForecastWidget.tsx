...
function getTrendCopy(payload: ForecastPayload | null): {
  label: string;
  tone: string;
  insight: string;
} {
  ...
  return {
    label: 'Price looks steady',
    tone: 'bg-[var(--tint-success)] text-[var(--color-success)]',
    insight: 'The model does not show a strong move, so current pricing can stay under review.',
  };
}
...
export default function PriceForecastWidget({ apiBase = 'http://localhost:3001' }: Props) {
  ...
  const trend = useMemo(() => getTrendCopy(data), [data]);
  ...
  async function refresh() {
    if (!token) {
      setError('Sign in as an admin or manager to load the forecast.');
      return;
    }
  ...
  async function runPredict() {
    if (!token) {
      setError('Sign in as an admin or manager to run a scenario.');
      return;
    }
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-xl border border-[var(--border)] bg-white px-4 py-3">
              ...
              <p className="mt-1 text-sm text-[var(--muted-foreground)]">{formatDate(data?.predictedAt)}</p>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            >
              {predicting ? 'Running scenario...' : 'Run scenario'}
            </button>
          </div>
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
          <div>
            ...
            <h3 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Test a business case</h3>
          </div>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {scenarioFields.map((field) => (
              <label key={field.key} className="space-y-2">
                ...
                />
                <span className="block text-xs text-[var(--muted-foreground)]">{field.helper}</span>
              </label>
            ))}
          </div>
