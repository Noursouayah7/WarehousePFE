...
export default function ReclamationsPage() {
  ...
  const fetchReclamations = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch reclamations');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-900">Reclamations</h1>
          <button
      ...
      {reclamations.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
          <p className="text-gray-500 text-lg">No reclamations yet</p>
        </motion.div>
      ) : (
            ...
            >
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900">
                    {reclamation.problemType.replace(/_/g, ' ')}
                  </p>
                </div>
              ...
              {reclamation.order && (
                <div className="mb-4 p-3 bg-gray-50 rounded">
                  <p className="text-sm font-semibold text-gray-700">Related Order</p>
              ...
              {reclamation.shipment && (
                <div className="mb-4 p-3 bg-gray-50 rounded">
                  <p className="text-sm font-semibold text-gray-700">Related Shipment</p>
                  <p className="text-sm text-gray-600">
                  ...
                  {reclamation.shipment.trackingNumber && (
                    <p className="text-sm text-gray-600">Tracking: {reclamation.shipment.trackingNumber}</p>
                  )}
                </div>
              )}
              ...
              {reclamation.managerNote && (
                <div className="p-3 bg-blue-50 rounded border border-blue-200">
                  <p className="text-sm font-semibold text-blue-700">Manager Note</p>
                  <p className="text-sm text-blue-900">{reclamation.managerNote}</p>
                </div>
              )}

              <div className="text-xs text-gray-500 mt-4 pt-4 border-t">
                Created: {new Date(reclamation.createdAt).toLocaleDateString()}
              </div>
            </motion.div>
          ))}
        </motion.div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          >
            <h2 className="text-2xl font-bold text-gray-900 mb-6">New Reclamation</h2>
                ...
                {attachmentUrl && (
                  <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm text-gray-700 truncate">{attachmentName ?? 'Selected attachment'}</p>
                      <button
                        type="button"
                        onClick={() => {
                          setAttachmentUrl(null);
