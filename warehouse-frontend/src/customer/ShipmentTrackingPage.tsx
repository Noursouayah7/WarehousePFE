...
export default function ShipmentTrackingPage() {
  ...
  const fetchShipments = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tx('Failed to fetch shipments'));
    } finally {
      setIsLoading(false);
    }
  }, [token]);
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">{tx('Shipment Tracking')}</h1>
        <p className="text-gray-600 mt-2">{tx('Track your orders and shipments in real-time')}</p>
      </motion.div>
      ...
      {shipments.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
          <p className="text-gray-500 text-lg">{tx('No shipments yet')}</p>
        </motion.div>
      ) : (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{tx('Qty')}: {shipment.quantity}</p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(shipment.status)}`}>
                      {tx(shipment.status.replaceAll('_', ' ').toLowerCase().replace(/^\w/, (letter) => letter.toUpperCase()))}
                    </span>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-semibold text-gray-900">{tx(shipment.type)}</p>
                </div>
              </div>

              {shipment.trackingNumber && (
                <div className="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
                  <p className="text-sm font-semibold text-blue-700">{tx('Tracking Number')}</p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                {shipment.expectedAt && (
                  <div>
                    <p className="text-sm text-gray-600">{tx('Expected Delivery')}</p>
                ...
                {shipment.receivedAt && (
                  <div>
                    <p className="text-sm text-gray-600">{tx('Received Date')}</p>
              ...
              {shipment.note && (
                <div className="mb-4 p-3 bg-gray-50 rounded border border-gray-200">
                  <p className="text-sm font-semibold text-gray-700">{tx('Notes')}</p>
                  <p className="text-sm text-gray-600">{shipment.note}</p>
                </div>
              )}

              <div className="text-xs text-gray-500 pt-4 border-t">
                {tx('Created')}: {new Date(shipment.createdAt).toLocaleDateString()}
              </div>
            </motion.div>
          ))}
        </motion.div>
