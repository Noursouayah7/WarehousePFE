...
export function SupportTicketsBoard({ canCreate, canUpdate }: { canCreate: boolean; canUpdate: boolean }) {
  ...
  useEffect(() => {
    ...
    void loadTickets(token)
      .catch((err: unknown) => {
        if (!mounted) return;
        setError(err instanceof Error ? err.message : tx('Failed to load support tickets'));
      })
      .finally(() => {
        if (!mounted) return;
        setIsLoading(false);
  ...
  async function submitTicket(event: FormEvent<HTMLFormElement>) {
    ...
    if (!title || !description) {
      setFormError(tx('Title and description are required.'));
      return;
    }
    ...
    try {
      ...
      setFormError(err instanceof Error ? err.message : tx('Failed to create support ticket'));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function changeStatus(ticketId: number, status: SupportTicketStatus, managerNote?: string) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tx('Failed to update ticket'));
    } finally {
  ...
  return (
    <section className="mt-8 rounded-2xl bg-transparent">
        ...
        <div className="fixed right-4 top-4 z-50 w-[min(24rem,calc(100vw-2rem))] rounded-2xl border border-[var(--border)] bg-[var(--card)] px-4 py-4 shadow-[0_24px_80px_rgba(15,23,42,0.18)] backdrop-blur-xl">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{tx('Ticket updated')}</p>
              <p className="mt-1 text-sm font-semibold text-slate-950">
                #{statusNotice.ticketId} {statusNotice.title}
              </p>
              <p className="mt-1 text-sm text-slate-600">
                {tx('Marked as')} {tx(STATUS_LABELS[statusNotice.status]).toLowerCase()}.
                {statusNotice.status === 'RESOLVED' ? ` ${tx('The issue is resolved.')}` : ` ${tx('The ticket is closed.')}`}
              </p>
            </div>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">{tx('Support tickets')}</h2>
          <p className="mt-1 text-sm text-[var(--muted-foreground)]">{visibleTickets.length} {tx('tickets')}</p>
        </div>
      </div>
      ...
      {canCreate && (
        <form onSubmit={submitTicket} className="mb-8 grid gap-3 rounded-2xl bg-[var(--card)] p-5 shadow-sm md:grid-cols-2">
          <div className="flex flex-col gap-2 md:col-span-2">
            <label className="text-xs font-medium text-[var(--muted-foreground)]">{tx('Title')}</label>
            <input
              value={form.title}
              onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))}
              className="rounded-xl border border-[var(--input)] bg-white px-4 py-3 text-sm outline-none focus:border-[var(--ring)]"
              placeholder={tx('Issue summary')}
            />
          </div>

          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-[var(--muted-foreground)]">{tx('Category')}</label>
            <select
              value={form.category}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-[var(--muted-foreground)]">{tx('Priority')}</label>
            <select
              value={form.priority}
          ...
          <div className="flex flex-col gap-2 md:col-span-2">
            <label className="text-xs font-medium text-[var(--muted-foreground)]">{tx('Description')}</label>
            <textarea
              value={form.description}
              onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))}
              rows={4}
              className="rounded-xl border border-[var(--input)] bg-white px-4 py-3 text-sm outline-none focus:border-[var(--ring)]"
              placeholder={tx('Explain the malfunction, delay, alert, or product issue')}
            />
          </div>
          ...
          >
            {saving ? tx('Creating ticket...') : tx('Submit ticket')}
          </button>
        </form>
      )}

      {isLoading ? (
        <div className="py-10 text-center text-sm text-[var(--muted-foreground)]">{tx('Loading tickets...')}</div>
      ) : (
          ...
          <table className="min-w-full border-separate border-spacing-y-2 text-sm">
            <thead>
              <tr className="text-left text-xs font-medium text-[var(--muted-foreground)]">
                ...
                <th className="px-3 py-2">{tx('TITLE')}</th>
                <th className="px-3 py-2">{tx('CATEGORY')}</th>
                <th className="px-3 py-2">{tx('PRIORITY')}</th>
                <th className="px-3 py-2">{tx('STATUS')}</th>
                <th className="px-3 py-2">{tx('CREATED BY')}</th>
                <th className="px-3 py-2">{tx('NOTE')}</th>
                {canUpdate && <th className="px-3 py-2">{tx('ACTIONS')}</th>}
              </tr>
            </thead>
            <tbody>
              {visibleTickets.map((ticket) => (
                <tr key={ticket.id} className="align-top">
                  ...
                  <td className="bg-[var(--card)] px-3 py-3 text-xs text-[var(--muted-foreground)]">{tx(CATEGORY_LABELS[ticket.category])}</td>
                  <td className="bg-[var(--card)] px-3 py-3 text-xs text-[var(--muted-foreground)]">{tx(PRIORITY_LABELS[ticket.priority])}</td>
                  <td className="bg-[var(--card)] px-3 py-3 text-xs text-[var(--muted-foreground)]">
                    <span className={["inline-flex rounded-full px-2.5 py-1 font-medium", STATUS_CLASSES[ticket.status]].join(' ')}>
                      {tx(STATUS_LABELS[ticket.status])}
                    </span>
                  </td>
                  <td className="bg-[var(--card)] px-3 py-3 text-xs text-[var(--muted-foreground)]">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {STATUS_OPTIONS.map((status) => (
                          <button
                            ...
                            onClick={() => changeStatus(ticket.id, status, status === 'RESOLVED' ? tx('Ticket resolved') : undefined)}
                          ...
                          >
                            {status === 'RESOLVED' ? tx('Mark resolved') : tx('Mark closed')}
                          </button>
                        ))}
                      </div>
                    </td>
